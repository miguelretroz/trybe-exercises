...
//   if (username !== validUser.username || password !== validUser.password) return res.status(401).json({ message: 'Invalid credentials!' });
...
const authMiddleware = (req, res, next) => {
  ...
  if (!username && !password) return res.status(401).json({ message: 'Username and password can\'t be blank!' });

  const foundUser = validUsers.find((user) => user.username === username);

  if (!foundUser) return res.status(401).json({ message: 'Invalid credentials!' });

  if (!(username === foundUser.username && password === foundUser.password)) return res.status(401).json({ message: 'Invalid credentials!' });
